 ...
 */
export const convertJsonToXml = (obj: any, rootName: string = 'root'): string => {
                ...
                if (value === null || value === undefined) {
                    ...
                    value.forEach(item => {
                        if (typeof item === 'object') {
                            ...
                            xml += `${indent}<${key}>${escapeXml(item.toString())}</${key}>\n`;
                        }
                    });
                } else {
 ...
 */
export const formatXmlResponse = (data: any, rootName: string = 'response'): string => {
    const response = {
        success: true,
        data: data
    };
