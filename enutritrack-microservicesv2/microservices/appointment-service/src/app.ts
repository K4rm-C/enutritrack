...
app.get("/health", async (req, res) => {
  const dbHealth = await AppDataSource.query("SELECT 1")
    .then(() => true)
    .catch(() => false);
  res.json({
    status: "OK",
    ...
    try {
      ...
      if (fecha) {
        ...
        query = query.andWhere(
          "cita.fechaHoraProgramada BETWEEN :fechaInicio AND :fechaFin",
          {
            fechaInicio,
            fechaFin,
          }
  ...
  async (req: any, res: express.Response) => {
    try {
      ...
      ResponseHandler.internalError(
        res,
        "Error obteniendo el detalle de la cita"
      );
    }
  }
);
    ...
    try {
      ...
      if (!estadoPendiente) {
        return ResponseHandler.internalError(
          res,
          "No se pudo encontrar el estado de cita por defecto"
        );
      }

      const nuevaCita = citaRepo.create({
        usuarioId:
          user.tipo === "patient" ? user.id : appointmentData.usuario_id,
  ...
  async (req: any, res: express.Response) => {
    try {
      ...
      ResponseHandler.created(
        res,
        signosGuardados,
        "Signos vitales agregados exitosamente"
      );
    } catch (error) {
...
app.get("/api/catalog/consultation-types", async (req, res) => {
  try {
    ...
    ResponseHandler.internalError(
      res,
      "Error obteniendo los tipos de consulta"
    );
  }
});
